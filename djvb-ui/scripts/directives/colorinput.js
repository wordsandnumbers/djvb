	...
	angular.module('djvbApp.directives.ColorInput', [])
		.directive('colorInput', function ($ionicPopover, constants) {
			return {
				...
				template: '<button class="button button-small button-light" title="Change user color" ng-style="{\'background-color\': color}" ng-click="pickColor($event)">Change</button>'
			};
			
			function ColorInputLink (scope, element, attrs, modelCtrl) {

				scope.userColors = ['#72C2FF', '#6798E6', '#CEA1E1', '#FFADED', '#FC809B', '#FFC787', '#FFF074', '#A6FC81', '#09E6AE', '#18C4C7',
